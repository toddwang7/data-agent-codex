...
function syncWindowStateFromSession(sessionId) {
  ...
  updateFileList();
  renderMessages();
  if (window.currentStage === STAGE_REPORT && window.latestWorkflowResult) {
...
function activateConfigModule(moduleId) {
  const targetButton = document.querySelector(`.module-item[data-module="${moduleId}"]`);
  const targetBlock = document.querySelector(`[data-module-block="${moduleId}"]`);
  if (targetButton) {
...
function renderReportEmpty() {
  ...
  document.getElementById("report-sections").innerHTML = '<div class="empty-state">最终报告会在确认分析计划后显示在这里。</div>';
...
function updateSummary(result, plan, execution) {
  ...
  reportingMonthElement.textContent = monthText;
...
function updateReportHeader(result, plan) {
  ...
  titleElement.textContent =
    variant === "single_month" ? `广告投放月报预览 · ${monthText || "单月"}` :
    variant === "multi_month" ? `广告投放跨月月报预览 · ${monthText || "跨月"}` :
    "广告数据分析预览";
  subtitleElement.textContent = `当前请求：${requestText}。右侧先承接结构化报告骨架，后续可以继续在对话中要求补充洞察、收紧表达或调整结构。`;
}
...
function resetWorkspaceState() {
  window.sessionStore[window.currentSessionId] = createEmptySessionState();
...
function renderWorkflowResult(result) {
  ...
  if (window.currentStage === STAGE_CLARIFICATION) {
    ...
    renderReportEmpty();
  } else if (window.currentStage === STAGE_PLAN) {
    currentMessages.push({
      role: "agent",
      kind: "plan_review",
      steps: plan.steps || []
...
async function analyzeFiles() {
  ...
  const taskType = requestText.includes("月报") ? "monthly_report" : "question_answering";
  ...
  statusText.textContent = window.currentStage === STAGE_PLAN
    ? "正在生成分析计划..."
...
function submitCurrentPrompt() {
  ...
  window.currentStage = STAGE_CLARIFICATION;
  persistCurrentSessionState();
  renderMessages();
  analyzeFiles();
}
